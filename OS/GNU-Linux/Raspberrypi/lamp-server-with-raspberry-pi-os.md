...
Note that it's best to not make use of an OS installed on the SD card, as you know, SD cards have a very limited lifetime.
...
We will create a new user with all user rights. This to avoid using the ``root`` user account already made.
...
I have had some issues with PHPMyAdmin, facing an annoying bug which gave me some errors on the PHPMyAdmin web interface:
